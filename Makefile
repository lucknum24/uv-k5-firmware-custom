...
ENABLE_TX_WHEN_AM             	?= 0
ENABLE_F_CAL_MENU             	?= 0
ENABLE_CTCSS_TAIL_PHASE_SHIFT 	?= 0  #使用标准CTCSS尾部相移，而不是QS独有的55Hz音调方法
ENABLE_BOOT_BEEPS             	?= 0  #在启动时为用户提供音频反馈，指示音量旋钮的位置
...
ifeq ($(ENABLE_FEAT_F4HWN),1)
	...
	AUTHOR_STRING_2 ?= BI3AYT
	VERSION_STRING_2 ?= v3.9.1
